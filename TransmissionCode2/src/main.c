...
void motorCode(uint16_t x, uint16_t y, uint16_t z, uint16_t w)
{
	...
	if(x<2252 && y<2252 && x>1843 && y>1843 && w==1)//360 left
	{
		w = w * 4095;
		TIM2 -> CCR1 = 0;//FL
		TIM2 -> CCR2 = (w*z)/25;//FR
	...
	if(x<2252 && y<2252 && x>1843 && y>1843 && w==3)//360 right
	{
		w = w - 2;
		w = w * 4095;
		TIM2 -> CCR1 = (w*z)/25;//FL
...
void UARTSetup(void)
{
	...
	USART1 -> BRR = 0xEA6;//57600 EA6 753
...
int main(void)
{
	uint16_t x=0, y=0, a=0, x1=0, y1=0, z=0, z1=0, w=0, w1=0;//Traversal Variables
	//uint16_t ;//Robotic Arm VAriables
